...
const getInterpolatedColor = (percentage) => {
  ...
  if (percentage > 66) {
    ...
    const r = Math.round(
      interpolation *
        (parseInt(primaryColor.substring(1, 3), 16) -
          parseInt(secondaryColor.substring(1, 3), 16)) +
        parseInt(secondaryColor.substring(1, 3), 16)
    );
    const g = Math.round(
      interpolation *
    ...
    const r = Math.round(
      interpolation *
        (parseInt(secondaryColor.substring(1, 3), 16) -
          parseInt(tertiaryColor.substring(1, 3), 16)) +
        parseInt(tertiaryColor.substring(1, 3), 16)
    );
    const g = Math.round(
      interpolation *
        (parseInt(secondaryColor.substring(3, 5), 16) -
          parseInt(tertiaryColor.substring(3, 5), 16)) +
        parseInt(tertiaryColor.substring(3, 5), 16)
    );
    const b = Math.round(
      interpolation *
        (parseInt(secondaryColor.substring(5, 7), 16) -
          parseInt(tertiaryColor.substring(5, 7), 16)) +
        parseInt(tertiaryColor.substring(5, 7), 16)
    );
    return `rgb(${r}, ${g}, ${b})`;
  }
    ...
    <>
      ...
      <div className={"pub-container"}>
        ...
        <div className={"pub-score-board"}>
          ...
          {roundOver && song && artists && (
            <div>
              <div className="song-info-block">
                <div
                  className="album-cover"
                  style={{
                    backgroundImage: "url(" + endRoundInfo.albumCover + ")",
                  }}
                ></div>
...
function playConfetti() {
  const explosionContainer: any = document.getElementById(
    "explosion-container"
  );
  for (let i = 0; i < 100; i++) {
    const confetti = createConfettiSvg(
      "rgb(" +
        Math.floor(Math.random() * 256) +
        "," +
        Math.floor(Math.random() * 256) +
          ...
          <tr key={index}>
            ...
            <td>
              <svg
                ...
                viewBox="0 0 512 512"
                className={`icon song-icon ${row.song ? "green" : ""}`}
              >
            ...
            <td>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className={`icon artist-icon ${row.artist ? "green" : ""}`}
                viewBox="0 0 448 512"
              >
            ...
            <td>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 448 512"
                className={`icon finish-icon ${row.first ? "green" : ""}`}
              >
