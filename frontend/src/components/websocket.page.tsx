...
class WebSocketComponent extends Component<socketProps, socketState> {
	constructor(props) {
		...
		const socket = new WebSocket(
			process.env.REACT_APP_WS_BACKEND_URL +
				"/ws/" +
		...
		socket.onmessage = (event) => {
			...
			if (messageData.type !== "game_update") console.log(messageData);
			if (messageData.type === "game_update") {
				...
				messageData.type === "seek_to_song" &&
				parseInt(messageData.user_id) ===
					parseInt(localStorage.getItem("userId") || "0")
			) {
				setPreviewUrl(messageData.preview_url);
	...
	componentWillReceiveProps(
		nextProps: Readonly<socketProps>,
		nextContext: any
	) {
